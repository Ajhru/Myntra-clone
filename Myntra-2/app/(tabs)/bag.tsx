...
const activeItems = bag?.filter((item: any) => item.status === "active");
const savedItems = bag?.filter((item: any) => item.status === "saved");
...
const total = activeItems?.reduce(
  ...
  try {
    await axios.put(
      `https://myntra-clone-xj36.onrender.com/bag/move/${itemId}`,
      { status: newStatus }
    );
    fetchproduct();
  } catch (error) {
    ...
    <View style={styles.itemInfo}>
      ...
      <View style={styles.quantityContainer}>
        <TouchableOpacity style={styles.removeButton}
          onPress={() => moveItem(item._id, "saved")}
        >
          <Text style={{ color: "#ff3f6c" }}>Save For Later</Text>
        </TouchableOpacity>
    ...
    {savedItems.map((item: any) => (
      <View key={item._id} style={styles.bagItem}>
        ...
        <View style={styles.itemInfo}>
          ...
          <TouchableOpacity
            onPress={() => moveItem(item._id, "active")}
          >
